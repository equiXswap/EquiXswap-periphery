...
import './interfaces/IEquixRouter02.sol';
...
contract EquixRouter02 is IEquixRouter02 {
    ...
    function _addLiquidity(
        ...
        if (IEquixFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IEquixFactory(factory).createPair(tokenA, tokenB);
        }
        (uint reserveA, uint reserveB) = EquixLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = EquixLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'EquixRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = EquixLibrary.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'EquixRouter: INSUFFICIENT_A_AMOUNT');
    ...
    function addLiquidity(
        ...
        address pair = EquixLibrary.pairFor(factory, tokenA, tokenB);
    ...
    function removeLiquidity(
        ...
        address pair = EquixLibrary.pairFor(factory, tokenA, tokenB);
        IEquixPair(pair).transferFrom(msg.sender, pair, liquidity); // send liquidity to pair
        (uint amount0, uint amount1) = IEquixPair(pair).burn(to);
        (address token0,) = EquixLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, 'EquixRouter: INSUFFICIENT_A_AMOUNT');
        require(amountB >= amountBMin, 'EquixRouter: INSUFFICIENT_B_AMOUNT');
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = EquixLibrary.sortTokens(input, output);
            uint amountOut = amounts[i + 1];
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOut) : (amountOut, uint(0));
            address to = i < path.length - 2 ? EquixLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IEquixPair(EquixLibrary.pairFor(factory, input, output)).swap(
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = EquixLibrary.sortTokens(input, output);
            IEquixPair pair = IEquixPair(EquixLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            ...
            amountOutput = EquixLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        require(
            ...
            'EquixRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) public pure virtual override returns (uint amountB) {
        return EquixLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    {
        return EquixLibrary.getAmountOut(amountIn, reserveIn, reserveOut);
    }
    ...
    {
        return EquixLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
    }
    ...
    function getAmountsOut(uint amountIn, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return EquixLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint amountOut, address[] memory path)
        ...
        returns (uint[] memory amounts)
    {
        return EquixLibrary.getAmountsIn(factory, amountOut, path);
    }
}
