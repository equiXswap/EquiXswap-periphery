...
export async function v2Fixture(provider: Web3Provider, [wallet]: Wallet[]): Promise<V2Fixture> {
  ...
  // deploy routers
  const router01 = await deployContract(wallet, EquixRouter01, [factoryV2.address, WETH.address], overrides)
  const router02 = await deployContract(wallet, EquixRouter02, [factoryV2.address, WETH.address], overrides)
  ...
  const WETHPair = new Contract(WETHPairAddress, JSON.stringify(IEquixPair.abi), provider).connect(wallet)
